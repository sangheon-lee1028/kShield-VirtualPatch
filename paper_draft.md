...
mock Ray Jobs API 환경에서 python3→sh→curl로 이어지는 2단계 공격 체인은 curl 실행 시점에(SHADOW_EXEC), bash `/dev/tcp/` 기반 우회 시도는 연결 시도 시점에(SHADOW_CONNECT) 각각 즉시 SIGKILL로 차단되었으며, 두 경우 모두 정상 job(echo, python3 -c 등)은 오탐 없이 통과하였다. 비활성/SHADOW_EXEC+CONNECT/LSM 사전 차단까지 포함한 3개 그룹을 10회씩 반복 측정한 결과, 세 그룹 간 통계적으로 유의미한 처리량·지연시간 차이는 관측되지 않았다(비활성 대비 SHADOW_EXEC+CONNECT: 처리량 p=0.0864, 지연시간 p=0.1400; SHADOW_EXEC+CONNECT 대비 LSM 추가: 처리량 p=0.6228, 지연시간 p=0.9348; Welch's t-test). 다만 SHADOW_EXEC/SHADOW_CONNECT는 모두 행위 발생 "이후" `bpf_send_signal`로 비동기 종료시키는 방식이라는 잔여 한계가 있어, 이를 해소하기 위해 `security_bprm_check_security`/`security_socket_connect` LSM 훅으로 시스템 콜 자체를 동기적으로 실패시키는 사전 차단 컴포넌트를 추가로 구현하고 VM에서 기능·성능 검증하였다(3.5절). 이 과정에서 실행 파일 블록리스트가 `/usr/bin/curl`은 막으면서 동일 파일을 가리키는 `/bin/curl`은 놓치는 버그를 실측으로 발견하였으며 — 셸의 `$PATH` 탐색이 거부된 경로 대신 다른 경로 문자열로 재시도한 것이 원인이었다 — 이는 이미 검증되었던 v3(SHADOW_EXEC)에도 동일하게 존재하던 누락이라 함께 수정하였다.
...
**한계 3 — fork 빈도가 낮은 워크로드만 사용**: 계보 추적(`sched_process_fork`/`sched_process_exit`)은 매 fork·exit마다 BPF map 갱신이 발생한다. 본 실험의 job은 fork 단계가 얕아(`sh`→`curl` 등 1~2단계), 실제 Ray처럼 태스크마다 워커 프로세스를 빈번히 새로 생성하는 워크로드에서의 오버헤드는 포착하지 못했을 수 있다.
...
VM 실측 결과, python3(워커)→sh→curl로 이어지는 2단계 공격 체인은 curl 실행 시점에(SHADOW_EXEC), bash `/dev/tcp/` 내장 기능을 이용한 우회 시도는 연결 시도 시점에(SHADOW_CONNECT) 각각 정확히 차단하였으며, 두 경우 모두 정상 job은 오탐 없이 통과함을 확인하였다. 비활성/SHADOW_EXEC+CONNECT/LSM 사전 차단을 포함한 3개 그룹을 10회씩 반복 측정한 결과, 어느 그룹 간 비교에서도 통계적으로 유의미한 처리량·지연시간 차이는 관측되지 않았다(off vs v3: 처리량 p=0.0864, 지연시간 p=0.1400; v3 vs v3+LSM: 처리량 p=0.6228, 지연시간 p=0.9348; Welch's t-test).
...
- **fork 집약적 워크로드에서의 재측정**: 현재 워크로드는 fork 단계가 얕다(1~2단계). 실제 Ray처럼 태스크마다 워커 프로세스를 빈번히 새로 생성하는 환경에서 계보 추적(`sched_process_fork`/`sched_process_exit`)의 오버헤드를 별도로 측정해야 한다.
