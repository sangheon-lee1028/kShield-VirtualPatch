...
```
[수정 전]
LSM_CONNECT_BLOCK 탐지! proc=event_engine parent=raylet dst=0.0.0.0:6379 => connect() -EPERM
(신규 워커가 뜰 때마다 반복 발생 — 정상 job 5개 + 악성 job 1개 전부 PENDING에서 정체)

[수정 후 재검증]
LSM_CONNECT_BLOCK 로그 전혀 발생하지 않음(GCS 연결 정상 통과)
LSM_EXEC_BLOCK 탐지! proc=sh parent=sh exec=/usr/bin/nc => execve() -EPERM
LSM_EXEC_BLOCK 탐지! proc=sh parent=sh exec=/bin/nc => execve() -EPERM

[실제 Jobs API 최종 결과]
정상 job 5개 → 전부 SUCCEEDED
악성 job(entrypoint: "nc -l -p 9999") → FAILED, exit code 126
