...
그럼에도 본 연구의 필요성은 유지된다. 첫째, 이 인증은 **기본값이 아니다** — Ray 개발사도 "인증을 기본값으로 전환하는 작업을 진행 중"이라고 공식적으로 밝히고 있어 [11], 현재 시점에는 기본으로 꺼져 있음을 자인한 셈이다. 둘째, 인증 메커니즘 자체도 완전하지 않다 — 이 토큰 인증이 막지 못하는 별도의 우회 경로(DNS 리바인딩을 통한 CORS 우회, CVE-2025-62593)가 2026년 8월 미국 CISA의 실제 악용 취약점(KEV) 목록에 오르기도 했다 [11]. 셋째, 가장 근본적으로 토큰 자체가 피싱이나 유출로 탈취되거나 정당한 사용자 계정이 침해되면 인증은 무력화되지만, 본 논문의 SHADOW_EXEC/SHADOW_CONNECT(3.3절)는 "공격자가 어떤 경로로 코드 실행 권한을 얻었는가"가 아니라 "그 권한으로 무엇을 하려 하는가"를 보므로, 인증 우회든 탈취된 정상 자격 증명이든 사후 행동 패턴이 같다면 동일하게 탐지·차단한다. 즉 인증과 본 논문의 행위 기반 탐지는 대체 관계가 아니라 서로 다른 계층을 담당하는 상호 보완적 방어다.
