...
> 3개 그룹)까지 VM에서 완료하였다. 이 과정에서 v3(3.3절)에도 있던 실행 파일
> 블록리스트 누락 버그를 함께 발견·수정하였다.
...
**한계 2 — 측정 범위가 여전히 "이 워크로드"로 국한됨**: `tcp_v4_connect`/`tcp_v6_connect` kprobe와 `security_socket_connect` LSM 훅은 AI 워커 계보 여부와 무관하게 **시스템의 모든 프로세스의 모든 connect() 호출**에서 실행되며(계보 판별을 위한 map 조회 자체가 매 호출마다 발생), 위 측정은 mock 서버 자신의 처리량·지연시간만 측정하였다. kShield-VirtualPatch와 무관한 다른 프로세스들이 동시에 활발히 네트워크 연결을 생성하는 상황에서 그 프로세스들의 connect() 지연시간이 얼마나 영향을 받는지는 측정하지 않았으며, 향후 연구 과제로 남긴다.
...
- **시스템 전역 오버헤드 측정**: SHADOW_CONNECT/LSM socket_connect 훅은 AI 워커 계보 여부와 무관하게 시스템의 모든 프로세스의 connect() 호출에서 실행된다(계보 판별 map 조회가 매 호출마다 발생). 현재 성능 측정(4.4절)은 mock 서버 자신의 처리량·지연시간만 다루며, kShield-VirtualPatch와 무관한 다른 프로세스들의 connect() 지연시간에 미치는 영향은 측정하지 않았다.
