...
> 완료하였다. 마지막으로 리뷰어 성격의 지적 세 가지(하드코딩된 rodata
...
본 절 전체(3.3, 3.5~3.9절)에 걸쳐 설계가 v1부터 v9까지 총 여덟 차례 반복되었다. 각 반복은 예외 없이 VM 실측이나 제출 전 자체 검토 과정에서 발견된 구체적 문제에 대한 대응이었으며, [표 1]은 이를 한눈에 볼 수 있도록 요약한 것이다 — 각 행의 상세 서술은 마지막 열의 절을 참고한다.
...
**남는 한계**: (1) `watched_parents[]`/`suspicious_bins[]`는 여전히 rodata로 남아 있어, 감시 대상 프로세스명·의심 바이너리 목록을 바꾸려면 재컴파일이 필요하다 — 신뢰 목적지 IP만큼 자주 바뀌는 값이 아니라는 판단으로 이번 개선에서는 범위 밖으로 두었다(5장 향후 연구). (2) 예외 처리는 UID 단위까지만 지원하며, 쿠버네티스 네임스페이스·cgroup 단위의 더 세밀한 예외는 아직 없다. (3) syslog 연동은 로컬 syslog 소켓에 메시지가 정확히 도달하는 것까지만 확인하였고, 실제 SIEM(Splunk, ELK 등)까지 도달해 파싱되는지는 로그 포워더 설정에 달려 있어 별도로 검증하지 않았다.
...
- **나머지 rodata 룰의 런타임 설정 지원**: `trusted_dst_ipv4[]`는 v8(3.9절)에서 BPF map + `kshield_ctl`로 런타임 갱신이 가능해졌다. 다만 `watched_parents[]`, `suspicious_bins[]`는 여전히 컴파일 타임 rodata라, 새로운 CVE 대응(감시 대상 프로세스명 추가 등)마다 재컴파일이 필요하다 — 신뢰 목적지 IP만큼 자주 바뀌는 값은 아니라고 판단해 이번 개선 범위 밖으로 두었다.
- **감시 예외의 세밀도 확장**: v9(3.9절)의 `exempt_uids_map`은 UID 단위까지만 지원한다. 쿠버네티스 네임스페이스·cgroup 단위의 더 세밀한 예외 처리(`bpf_get_current_cgroup_id()` 기반)는 향후 과제로 남는다.
