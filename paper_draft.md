...
본 논문은 이 개념을 AI 서빙 프레임워크의 커널 수준으로 확장한다. eBPF를 이용하면 애플리케이션 코드를 한 줄도 수정하지 않고 실행 중인 프로세스에 탐지 로직을 "붙였다 뗄 수 있다." 기본 방어선인 SHADOW_EXEC/SHADOW_CONNECT(3.3절)는 이 무재부팅(no-restart) 성질을 배포 환경과 무관하게 유지한다. 동기적 사전 차단을 제공하는 LSM 컴포넌트(3.5절)의 사정은 다르다. 활성 LSM 스택에 `bpf`가 이미 포함된 배포판·커널에서는 같은 방식으로 재부팅 없이 적용되지만, 포함되어 있지 않은 구형 배포판이나 사전 설정이 안 된 노드에서는 부팅 파라미터 변경과 최초 1회 재부팅이 필요하다. 무재부팅 성질은 3.3절 방어선에는 무조건 성립하지만, 3.5절 LSM 계층에는 배포 환경에 따라 조건부로만 성립하는 셈이다 — 두 컴포넌트를 독립된 계층으로 유지하는 이유이기도 하다(3.5절 상세 논의 참고). 이는 정식 패치가 배포되기 전까지의 공백을 메우는 임시 방어선이자, 정식 패치가 나온 뒤에는 훅을 제거하기만 하면 되는 저비용 대응 수단이 된다.
...
기존 도구 위에 룰을 얹는 대신 별도의 최소 구현을 처음부터 작성한 것은, 룰 갱신·알림 연동 같은 프로덕션 편의 기능을 갖추는 것보다 "AI 워커 계보 추적 + exec/connect 이중 계층 + 동기적 사전 차단"이라는 탐지 메커니즘 자체를, 연구 목적에 맞게 외부 의존성 없이 정확히 통제된 형태로 구현·측정하는 것을 우선했기 때문이다. 이는 실용적 배포 도구로서는 Falco/Tetragon 대비 열위인 지점(재컴파일 없는 룰 갱신, SIEM 연동, 다중 커널 버전 검증 등은 향후 연구로 남김, 5장)이다.

같은 판정(AI 워커 계보의 신뢰되지 않은 connect/exec)을 Falco 룰과 Tetragon 정책으로도 구성해, 같은 VM·같은 워크로드에서 kShield-VirtualPatch와 오버헤드·탐지 방식을 비교했다(4.5절). 세 도구 모두 판정 자체는 동일하게 내린다 — 그 점은 실험 설계상 당연하다. 갈리는 지점은 차단 시점과 비용이다. kShield는 두 워크로드에서 기준(도구 없음) 대비 오버헤드가 통계적으로 구분되지 않거나(TOST 동등성 입증) 유의하더라도 사전에 고정한 ±2% 마진 안에 머물렀다. Falco와 Tetragon은 두 경우 모두 마진을 벗어나는 유의한 오버헤드를 보였다. 이는 범용 관측 에이전트(넓은 이벤트 범위, 사용자 공간 필터링 또는 전체 프로세스 생명주기 추적)와, 좁은 판정 하나만 커널 안에서 수행하는 최소 구현 간의 비용 차이로 해석된다.
...
본 절 전체(3.3, 3.5~3.13절)에 걸쳐 설계가 v1부터 v13까지 총 열두 차례 반복되었다. 각 반복은 예외 없이 VM 실측이나 제출 전 자체 검토 과정에서 발견된 구체적 문제에 대한 대응이었으며, [표 1]은 이를 한눈에 볼 수 있도록 요약한 것이다 — 각 행의 상세 서술은 마지막 열의 절을 참고한다.
...
이 취약점은 벤더가 "의도된 설계"라는 입장을 고수하며 즉각적인 공식 패치를 제공하지 않거나 [2], 조직 내부적으로 프레임워크 업그레이드에 따른 프로덕션 호환성 리스크 때문에 패치 적용이 장기간 지연되는 상황을 전제로 한다. 실제로 Oligo Security의 조사에 따르면 [1] 수천 대의 Ray 클러스터가 이 취약점을 통해 실제로 침해되어 암호화폐 채굴, 역방향 셸 연결, 클러스터 내 자격 증명 탈취 등에 악용된 사례가 확인되었다. 이는 과거형 사례가 아니다 — 2025년 11월에도 동일한 CVE-2023-48022를 악용하는 "ShadowRay 2.0" 캠페인이 보고되었다 [10]. 공격자는 OAST 기반 정찰 서비스(interact.sh)로 인터넷 전역을 스캔해 새로 취약한 Ray 대시보드를 찾아내고, 일단 한 클러스터에 침투하면 Ray 자체의 노드 스케줄링 기능(`NodeAffinitySchedulingStrategy`)을 악용해 그 클러스터 안의 모든 노드로 자동 확산시킨다 — "자가 전파형"이라는 이름은 피해자의 정상 오케스트레이션 기능이 전파 도구로 무기화되었다는 뜻이지, 클러스터 사이를 감염이 건너뛰는 웜(worm) 방식이라는 뜻은 아니다. 노출된 Ray 서버는 23만 대 이상으로 집계되었다 [10].

**완화책으로서의 토큰 인증과 그 한계**: Ray는 2.52.0부터 선택적(opt-in) 토큰 인증(`RAY_AUTH_MODE=token`)을 제공한다 [11]. 이 인증이 본 논문의 위협 모델을 실제로 무력화하는지 VM에 Ray 2.52.0을 직접 설치해 검증하였다 — 인증을 켠 상태에서 `attack/exploit_shadowray.py`(토큰 없이 job 제출)를 실행하면 `HTTP 401 Unauthorized`로 요청 자체가 거부되어 코드가 실행되지 않았고, 올바른 토큰을 첨부하면 동일한 API 호출이 정상 처리됨을 확인하였다(대조군). 즉 이 인증은 본 논문이 다루는 공격 경로를 실제로 차단한다.

그럼에도 본 연구의 필요성은 유지된다. 첫째, 이 인증은 **기본값이 아니다** — Ray 개발사도 "인증을 기본값으로 전환하는 작업을 진행 중"이라고 공식적으로 밝히고 있어 [11], 현재 시점에는 기본으로 꺼져 있음을 자인한 셈이다. 둘째, 이 토큰 인증이 도입되기 전 버전에는 인증에 준하는 보호 장치조차 없었고, 그 구버전이 지금도 실제로 공격받고 있다 — 2.52.0 이전 Ray는 브라우저發 요청을 막기 위해 `User-Agent` 헤더가 `Mozilla`로 시작하는지만 확인하는 약한 방식을 썼는데, 이는 DNS 리바인딩으로 우회 가능해 악성 웹페이지 방문만으로도 로컬에 떠 있는 Ray 대시보드에 셸 명령을 실행시킬 수 있었다(CVE-2025-62593). 이 취약점은 2.52.0에서 고쳐졌고 토큰 인증도 이 공격을 실제로 막지만 [11], 2026년 8월 CISA가 이를 KEV(실제 악용 취약점) 목록에 올렸다는 것은 아직 2.52.0 이전 버전을 쓰는 조직이 실제로 공격받고 있다는 뜻이다 — 본 논문이 전제하는 "패치·업그레이드가 지연되는 환경"이 인증 계층에서도 그대로 반복되고 있다는 근거다. 셋째, 가장 근본적으로 토큰 자체가 피싱이나 유출로 탈취되거나 정당한 사용자 계정이 침해되면 인증은 무력화되지만, 본 논문의 SHADOW_EXEC/SHADOW_CONNECT(3.3절)는 "공격자가 어떤 경로로 코드 실행 권한을 얻었는가"가 아니라 "그 권한으로 무엇을 하려 하는가"를 보므로, 인증 우회든 탈취된 정상 자격 증명이든 사후 행동 패턴이 같다면 동일하게 탐지·차단한다. 즉 인증과 본 논문의 행위 기반 탐지는 대체 관계가 아니라 서로 다른 계층을 담당하는 상호 보완적 방어다.
...
**수정**:
- `trusted_dst_ipv4[]` rodata 배열을 `trusted_dst_ipv4_map`이라는 BPF 해시맵으로 바꾸고, 유저스페이스 로더가 이를 `/sys/fs/bpf/kshield_trusted_ips_{v3,lsm}`에 핀(pin)한다. 신뢰 목적지의 add/del/list는 새로 만든 `kshield_ctl` 유틸리티가 담당하며, 데몬 재시작·재컴파일이 필요 없다. 부수 효과로 맵이 핀되어 있어 데몬이 재시작돼도 신뢰 목적지 목록이 유지된다.
- 두 데몬 모두에 `--syslog` 플래그를 추가하였다. 켜면 탐지 이벤트를 사람이 읽는 콘솔 출력에 더해 `syslog(LOG_AUTHPRIV)`로 JSON 한 줄씩 남긴다 — 이미 대부분의 기업 환경에 있는 rsyslog/journald → SIEM 파이프라인에 그대로 올라탈 수 있다(Falco 등 기존 런타임 보안 도구도 syslog를 출력 채널 중 하나로 지원하는 것과 동일한 접근). `comm`/`parent_comm`/`filename`은 공격자가 통제하는 값이므로, JSON에 삽입하기 전에 이스케이프한다(로그 인젝션 방지).
- `exempt_uids_map`이라는 별도 BPF 해시맵(UID → 1)을 추가하고, `current_is_watched()`의 최우선 조건으로 확인한다. 이 맵에 있는 UID는 계보·자기자신 판정과 무관하게 감시 자체가 완전히 건너뛰어진다(이벤트도 남지 않는다). `kshield_ctl exempt-add/exempt-del/exempt-list`로 런타임에 관리한다.
- `kshield_ctl`은 리소스가 두 종류(신뢰 IP, 예외 UID)가 되면서 `trust-add/trust-del/trust-list`, `exempt-add/exempt-del/exempt-list` 여섯 개 하위 명령으로 재구성하였다. v3·LSM은 서로 다른 BPF 오브젝트라 맵을 공유하지 않으므로, 기본 동작(`--target both`)은 두 맵 모두에 동시 적용한다.

**VM 실측 결과**: mock Ray Jobs API로 신뢰 안 된 목적지(`1.1.1.1`)로의 curl job을 6회 제출하며, 사이사이에 `trust-add`/`trust-del`, `exempt-add`/`exempt-del`을 끼워 넣어 재현하였다.

```
[시도1, 예외 없음]                     → 차단(Killed), SHADOW_CONNECT 탐지
$ sudo ./kshield_ctl trust-add 1.1.1.1
[시도2, IP 신뢰 등록 후]                → 통과(로그 없음)
$ sudo ./kshield_ctl trust-list
  1.1.1.1
$ sudo ./kshield_ctl trust-del 1.1.1.1
[시도3, IP 신뢰 해제 후]                → 차단(Killed), SHADOW_CONNECT 탐지
[시도4, UID 예외 없음]                  → 차단(Killed), SHADOW_CONNECT 탐지
$ sudo ./kshield_ctl exempt-add 1009
[시도5, UID 예외 등록 후]               → 통과(로그 없음)
$ sudo ./kshield_ctl exempt-list
  uid=1009
$ sudo ./kshield_ctl exempt-del 1009
[시도6, UID 예외 해제 후]               → 차단(Killed), SHADOW_CONNECT 탐지

$ sudo journalctl -t kshield_vpatch -n 20 --no-pager
Sep 07 16:08:13 ... {"tool":"kshield_vpatch","event":"SHADOW_CONNECT","parent_comm":"sh","comm":"curl","pid":838948,"dst_ip":"1.1.1.1","dst_port":80,"enforced":true}
Sep 07 16:08:20 ... {"tool":"kshield_vpatch","event":"SHADOW_CONNECT","parent_comm":"sh","comm":"curl","pid":838964,"dst_ip":"1.1.1.1","dst_port":80,"enforced":true}
Sep 07 16:08:33 ... {"tool":"kshield_vpatch","event":"SHADOW_CONNECT","parent_comm":"sh","comm":"curl","pid":838971,"dst_ip":"1.1.1.1","dst_port":80,"enforced":true}
Sep 07 16:08:34 ... {"tool":"kshield_vpatch","event":"SHADOW_CONNECT","parent_comm":"sh","comm":"curl","pid":838988,"dst_ip":"1.1.1.1","dst_port":80,"enforced":true}
```
...
**VM 실측 결과**: 데몬을 최초 기동(핀 경로가 전부 새로 생성되는 상황)한 뒤, 다음을 순서대로 확인하였다.

```
[기본값 시드 확인] echo benign-job → 오탐 없음 / nc 1.1.1.1 80 → SHADOW_EXEC 탐지, Killed

$ sudo ./kshield_ctl parent-add myworker
$ cp /bin/bash /tmp/myworker && /tmp/myworker -c 'curl http://1.1.1.1/ | cat' &
[18:18:45] SHADOW_CONNECT 탐지! parent=myworker -> proc=curl(...) dst=1.1.1.1:80 => SIGKILL 전송
$ sudo ./kshield_ctl parent-del myworker
$ /tmp/myworker -c 'curl http://1.1.1.1/ | cat' &     → 로그 없음(정상 통과)

$ sudo ./kshield_ctl bin-add /usr/bin/telnet
$ (mock job) telnet 1.1.1.1 80 → SHADOW_EXEC 탐지! exec=/usr/bin/telnet, Killed
$ sudo ./kshield_ctl bin-del /usr/bin/telnet
$ (mock job) telnet 1.1.1.1 80 → SHADOW_EXEC은 안 뜨지만
  SHADOW_CONNECT 탐지! parent=sh -> proc=telnet dst=1.1.1.1:80 => SIGKILL 전송
  (exec 계층에서 빠져도 connect 계층이 defense-in-depth로 여전히 방어)

$ sudo ./kshield_ctl self-add myworker2
$ cp /usr/bin/curl /tmp/myworker2 && timeout 5 /tmp/myworker2 http://1.1.1.1/
[18:55:12] SHADOW_CONNECT 탐지! parent=timeout -> proc=myworker2(...) dst=1.1.1.1:80 => SIGKILL 전송  (종료 코드: 137)
$ sudo ./kshield_ctl self-del myworker2
...
```
[데몬 기동] /sys/fs/bpf/kshield_link_{lineage_fork,lineage_exit,shadow_exec,
            shadow_connect_v4,shadow_connect_v6} 5개 핀 파일 생성 확인

[kill -9] $ sudo kill -9 $KSPID
$ sudo bpftool prog list | grep -A1 'shadow\|lineage'
  → 5개 프로그램(trace_lineage_fork/exit, trace_shadow_exec,
    trace_shadow_connect_v4/v6) 전부 잔존 확인

[데몬 없는 상태에서 공격 재현] nc -w 2 203.0.113.1 80
  → nc가 자신의 2초 타임아웃보다 먼저 사라짐(1초 이내) — 커널에 남은
    프로그램이 여전히 SIGKILL을 보내고 있음을 시사

[SIGTERM으로 정상 종료] $ sudo kill $KSPID2   (kill -9 아님)
  → 종료 로그: 정상 종료 메시지("Done", "Killed" 아님)
$ sudo ls /sys/fs/bpf/ | grep kshield_link
  → 핀 파일 전부 사라짐(정상 종료 시 실제로 보호가 꺼짐을 확인)
```

즉 (1) 크래시(`kill -9`)에서는 커널에 남은 프로그램이 계속 보호하고, (2) 의도된 종료(SIGTERM)에서는 실제로 보호가 꺼진다는 두 요구사항 모두 VM에서 확인하였다.
...
```
[데몬 기동] 핀 파일 6개(기존 5개 + kshield_link_lineage_selfexec) 생성 확인

[파이프 없는 단일 명령 self-exec, 수정 전에는 무탐지였던 케이스]
$ /tmp/raylet -c "nc -w 2 203.0.113.1 80"
Killed
소요시간: 0.006초 (수정 전 2.008초 → 수정 후 즉시 차단)
...
[데몬 로그]
SHADOW_EXEC 탐지! parent=bash(pid=8279) -> child=nc(pid=17532) exec=/usr/bin/nc => SIGKILL 전송
SHADOW_EXEC 탐지! parent=raylet(pid=17539) -> child=nc(pid=17540) exec=/usr/bin/nc => SIGKILL 전송
```

첫 번째 로그 줄(`parent=bash`)이 self-exec 케이스다 — `parent_comm`은 BPF 코드가 항상 `real_parent`(이 프로세스를 최초로 fork한 로그인 셸)의 comm을 읽으므로 `raylet`가 아닌 `bash`로 나타나며, 이는 로그 표기 방식일 뿐 탐지 로직의 오류가 아니다. 두 번째 줄(`parent=raylet`)은 기존 fork 강제 케이스로, 이번 수정과 무관하게 원래부터 정상 동작하던 경로다. 두 경로 모두 정확히 한 번씩만 로그가 남아 중복 탐지나 누락이 없음을 확인하였다.

**남는 한계**: (1) `execve()`만 다루며, `execveat()`(예: `fexecve()`를 통한 실행)를 통한 동일한 self-exec 치환은 별도 훅이 없어 여전히 놓칠 수 있다. (2) 이번 검증도 단일 세션 1회 실행 기반이며, N회 반복 확인은 아직 없다.
...
GCS IP를 `kshield_ctl trust-add`로 등록하자 raylet은 정상적으로 기동되었으나, 이번에는 Ray가 새로 생성하는 워커 프로세스(`python3.11`)가 간헐적으로 여전히 SIGKILL당하는 현상이 남았다. 데몬 로그에는 매번 목적지가 `0.0.0.0`으로 찍혔는데, 신뢰 목록에 `0.0.0.0`도 이미 등록해 두었음에도 계속 차단되는 모순이 있었다. `bpf_printk` 기반 임시 커널 디버그 출력(진단 후 제거)으로 직접 확인한 결과, 이 이벤트는 IPv4 훅(`trace_shadow_connect_v4`)이 아니라 IPv6 훅(`trace_shadow_connect_v6`)에서 발생하고 있었고, 실제 목적지는 `::ffff:223.195.109.35`(GCS의 실제 IPv4 주소를 감싼 IPv4-mapped IPv6 주소)였다 — Ray의 gRPC 클라이언트가 같은 목적지에 IPv6 소켓으로도 접속을 시도하고 있었던 것이다. 로그에 목적지가 `0.0.0.0`으로 잘못 찍힌 것은, IPv6 이벤트도 유저스페이스가 IPv4 전용 필드(`evt.dst_addr`, IPv6 훅에서는 채워지지 않아 0으로 남음)로 출력하기 때문임을 함께 확인하였다(별도의 로그 표시 결함). `trace_shadow_connect_v6`는 애초에 IPv6 목적지용 신뢰 목록이 전혀 없어(loopback `::1`만 예외) 이 경로가 무조건 차단되었던 것이며, 같은 디버그 출력으로 IPv4 훅 쪽은 해당 목적지를 `trusted=1`로 정확히 인식하고 있음도 함께 확인해, IPv4 신뢰 로직 자체는 처음부터 올바르게 동작하고 있었음을 검증하였다.

**수정**: `trace_shadow_connect_v6`에서 목적지 주소가 `::ffff:0:0/96`(IPv4-mapped IPv6) 대역인지 확인하고, 해당하면 뒤 4바이트에 담긴 실제 IPv4 주소를 꺼내 기존 `trusted_dst_ipv4_map`으로 그대로 재판정하도록 하였다 — 별도의 IPv6 전용 신뢰 목록을 새로 만들지 않고 이미 검증된 IPv4 신뢰 로직을 재사용하였다. 순수 IPv6 목적지(IPv4-mapped가 아닌 경우)에 대한 신뢰 목록은 여전히 없다.

**VM 실측 결과**: 수정 후 재빌드하여 실제 Ray 클러스터로 전 과정을 재현하였다. (참고: Python 3.12 환경에서는 Ray 자체의 대시보드 서브프로세스가 원인 불명으로 기동에 실패해, kShield-VirtualPatch와 무관한 환경 이슈로 판단하여 Python 3.11 가상환경으로 우회하였다.)

```
[GCS IP 신뢰 등록 후 클러스터 기동]
$ ray start --head
--------------------
Ray runtime started.
--------------------

[정상 job 5회 제출, 실제 Jobs API(/api/jobs/, CVE-2023-48022가 노리는 그 엔드포인트)]
entrypoint: python3 -c "print(N)" (N=1~5)
→ 5개 전부 SUCCEEDED

[악성 job 제출, 같은 API로 재현]
entrypoint: nc -w 2 203.0.113.1 80
→ FAILED, exit code 137(SIGKILL)

[데몬 자체 로그]
SHADOW_EXEC 탐지! parent=sh(pid=72968) -> child=nc(pid=72970)
  exec=/usr/bin/nc => SIGKILL 전송
```
...
4.5절의 Falco/Tetragon 비교 실험은 별도 세션(커널 `6.8.0-139-generic`, Falco 0.45.0)에서 수행하였다 — 정확한 환경 값은 4.5절에 별도로 명시한다. 그룹 간 상대 비교는 같은 세션 안에서 이루어지므로 유효하지만, 위 표의 절대 수치와 4.5절의 절대 수치를 서로 비교해서는 안 된다(한계 1 참고).
...
**여기서는 처음으로 통계적으로 유의미한 차이가 관측되었다.** off 대비 v3_lsm는 처리량 −0.67%(605.56→601.51 req/s), 지연시간 +0.7%(1.582→1.593ms)로 그 자체는 작지만, 표준편차가 매우 작아(±2 수준) N=10에서도 강한 유의성(p<0.001)으로 검출되었다. v3 단독 대비 v3_lsm 추가분은 유의하지 않아(p=0.16~0.19), LSM 계층 자체가 오버헤드를 크게 늘리기보다는 v3(SHADOW_EXEC+CONNECT)의 계보 추적 비용이 fork 빈도가 높아지며 누적되어 통계적으로 드러난 것으로 해석된다. 이는 표 4/6(얕은 fork)에서 "유의미한 차이 없음"으로 나온 결과와 모순되지 않는다 — **오버헤드가 워크로드의 fork 빈도에 비례**하며, fork가 드문 워크로드에서는 노이즈에 묻혀 보이지 않다가 fork가 빈번해지면 드러난다는, 더 정확한 그림을 보여준다. 대략적으로(끝단(end-to-end) HTTP 지연시간 기준의 거친 추정치이며 개별 fork/exit 훅만 격리 측정한 것은 아님) job당 약 50회 늘어난 fork+exit 쌍에 대해 11마이크로초의 추가 지연이 관측되어, 추가 fork+exit 쌍 하나당 약 0.2마이크로초 수준의 비용으로 추산된다 — 이는 BPF map 조회 몇 번과 문자열 비교 수준의 훅 치고는 합리적인 크기다.

**정밀 per-fork 비용 재검증**: 위 0.2마이크로초 추정치를 검증하기 위해, `trace_lineage_fork`/`trace_lineage_exit` 두 훅 자체에 `bpf_ktime_get_ns()` 기반 진입/종료 타임스탬프를 심어 훅 실행 시간만 커널 내부에서 직접 격리 측정하는 `hook_timing_map`(PERCPU_ARRAY)을 추가하고, 이를 읽는 `kshield_ctl fork-timing-stats` 명령을 구현하였다. 데몬을 새로 기동한 뒤 위와 동일한 fork 집약적 워크로드(`benchmark_vpatch.py --fork-heavy`, 500회)를 재현하여 측정한 결과, `trace_lineage_fork`는 13,528회 호출에 평균 1039.8ns, `trace_lineage_exit`는 13,528회 호출에 평균 680.6ns가 걸려, fork+exit 한 쌍의 순수 훅 실행 비용은 약 1.72마이크로초로 나타났다 — 위 end-to-end 추정치(약 0.2마이크로초)보다 약 8~9배 크다. 이 차이는 기존 추정 방식이 HTTP 왕복 지연시간이라는 훨씬 거친 단위(밀리초)에서 역산한 값이라, 마이크로초 단위의 실제 훅 비용을 안정적으로 분리해내지 못했음을 보여준다 — 즉 기존 추정치는 방향(오버헤드가 존재하고 fork 빈도에 비례한다는 것)은 맞았지만 절대 크기는 과소평가하고 있었다. 호출 횟수(13,528회)가 이론상 최대치(job당 최대 52회 × 500 = 26,000회)의 약 절반인 이유는 자원 고갈이 아니라, `FORK_HEAVY_ENTRYPOINTS`에 등록된 두 entrypoint 중 하나(`echo x > /dev/null` 반복)가 `echo`를 외부 바이너리가 아닌 셸 내장 명령으로 실행해 fork를 거의 유발하지 않기 때문이다(job당 약 2회) — 나머지 하나(`/bin/true` 반복, job당 약 52회)와 50:50으로 섞이며 평균이 약 27회/job으로 낮아진 것이며, 이는 워크로드 구성상 원래 의도된 동작이다.
...
### 4.5 Falco·Tetragon과의 비교 실험

2.3절에서 인정한 공백 — "기존 eBPF 런타임 보안 도구와의 정량 비교를 아직 수행하지 못했다" — 을 메우기 위해, kShield-VirtualPatch의 SHADOW_CONNECT/SHADOW_EXEC와 같은 판정(AI 워커 계보에 속한 프로세스가 loopback 밖 IPv4로 connect를 시도하거나 `nc`/`ncat`을 exec)을 Falco 룰[5]과 Tetragon 정책[6]으로 각각 옮겨, 같은 VM·같은 세션·같은 워크로드에서 비교하였다. Falco는 modern eBPF 드라이버(0.45.0)로, Tetragon은 standalone 에이전트로 각각 기본 설정으로 실행하였으며, 두 도구 모두 커스텀 룰/정책 외의 부가 기능(SIEM 연동, 대시보드 등)은 켜지 않았다. off(도구 없음)를 포함한 네 그룹을 도구를 먼저 기동한 뒤 mock 서버를 띄우는 순서로 구성해, 모든 도구가 계보를 처음부터 동일하게 관찰하도록 통제하였다.

**측정 설계상의 특이점**: 탐지 실험의 목적지는 애초 외부 IP(`1.1.1.1`)였으나, 실험 VM이 외부망에 도달하지 못해 응답 없는 목적지로의 블로킹 connect(`bash /dev/tcp`)가 시나리오 제한 시간 안에 반환되지 않는 문제를 실측으로 발견하였다. Falco 룰은 `connect` 반환 시점에 평가되므로 이 조건에서 탐지를 놓쳤으나, 이는 도구의 한계가 아니라 실험 환경의 결함이었다. VM 자신의 비 loopback IPv4 주소에 하네스가 여는 TCP 수신기(Sink)로 목적지를 바꿔 해소하였다 — 세 도구 모두 `127.0.0.0/8` 밖의 주소를 신뢰되지 않은 목적지로 판정하므로 기준 자체는 바뀌지 않는다. 아울러 Tetragon 이벤트 스트림이 gRPC 소켓 생성 이전에 시작되면 즉시 종료되어 정책이 실제로는 SIGKILL을 실행하면서도 탐지가 0으로 기록되는 계측 결함도 발견해, 소켓 생성과 스트림 생존을 확인한 뒤에야 측정을 시작하도록 하네스를 수정하였다.

**[표10] 탐지·차단 비교 (N=10 반복, 기준 off)**

| 그룹 | 시나리오 | 탐지 | SIGKILL | Sink 연결 수립 | 유출 바이트(회당) |
|------|---------|------|---------|--------------|-------------------|
| off | curl_untrusted | 0/10 | 0/10 | 10/10 | 83 |
| off | bash_devtcp | 0/10 | 0/10 | 10/10 | 7 |
| off | nc_untrusted | 0/10 | 0/10 | 10/10 | 0 |
| kShield-VirtualPatch | curl_untrusted | 10/10 | 10/10 | 10/10 | 0 |
| kShield-VirtualPatch | bash_devtcp | 10/10 | 10/10 | 10/10 | 0 |
| kShield-VirtualPatch | nc_untrusted | 10/10 | 10/10 | 0/10 | 0 |
| Falco | curl_untrusted | 10/10 | 0/10 | 10/10 | 83 |
| Falco | bash_devtcp | 10/10 | 0/10 | 10/10 | 7 |
| Falco | nc_untrusted | 10/10 | 0/10 | 10/10 | 0 |
| Tetragon | curl_untrusted | 10/10 | 10/10 | 10/10 | 0 |
| Tetragon | bash_devtcp | 10/10 | 10/10 | 10/10 | 0 |
| Tetragon | nc_untrusted | 10/10 | 10/10 | 10/10 | 0 |

정상 job(`benign_echo`)은 네 그룹 모두 탐지 0/10·SIGKILL 0/10으로 오탐이 없었다(별도 행 생략). 세 도구가 같은 판정을 내리는 것은 룰/정책을 kShield-VirtualPatch와 동등하게 이식했으므로 당연하다 — 이 표에서 실제로 갈리는 것은 판정 결과가 아니라 **차단 시점**이다. kShield-VirtualPatch와 Tetragon은 유출 바이트가 모두 0으로, 첫 데이터가 나가기 전에 차단하였다. Falco는 탐지 전용이라 off와 동일하게 데이터가 그대로 나갔다. `nc`의 Sink 연결 수립이 kShield-VirtualPatch에서만 0/10인 것은 SHADOW_EXEC이 exec 단계에서 먼저 차단하기 때문이며, Tetragon 정책에는 이에 대응하는 exec 계층이 없어 connect 단계에서 차단한다(연결은 수립되지만 유출은 0).

**[표11] 성능 오버헤드 — normal 워크로드 (N=20, 기준 off, Bonferroni α=0.0042, 동등성 마진 ±2%, TOST α=0.05)**

| 그룹 | 지표 | 평균 ± SD | Δ% | 95% CI | Welch p | TOST p | 판정 |
|------|------|-----------|-----|--------|---------|--------|------|
| off | 처리량(req/s) | 753.27 ± 3.63 | (기준) | | | | |
| Falco | 처리량(req/s) | 721.53 ± 4.34 | −4.21% | [−4.55, −3.87] | 0.0000 | 1.0000 | 차이 있음 |
| kShield-VirtualPatch | 처리량(req/s) | 749.69 ± 3.13 | −0.48% | [−0.76, −0.19] | 0.0019 | 0.0000 | 유의하나 마진 이내 |
| Tetragon | 처리량(req/s) | 729.69 ± 4.99 | −3.13% | [−3.50, −2.76] | 0.0000 | 1.0000 | 차이 있음 |
| off | 지연시간(ms) | 1.248 ± 0.006 | (기준) | | | | |
| Falco | 지연시간(ms) | 1.307 ± 0.008 | +4.67% | [+4.31, +5.03] | 0.0000 | 1.0000 | 차이 있음 |
| kShield-VirtualPatch | 지연시간(ms) | 1.255 ± 0.005 | +0.50% | [+0.22, +0.79] | 0.0010 | 0.0000 | 유의하나 마진 이내 |
| Tetragon | 지연시간(ms) | 1.291 ± 0.009 | +3.39% | [+2.99, +3.79] | 0.0000 | 1.0000 | 차이 있음 |

**[표12] 성능 오버헤드 — fork 집약적 워크로드 (N=20, 기준 off)**

| 그룹 | 지표 | 평균 ± SD | Δ% | 95% CI | Welch p | TOST p | 판정 |
|------|------|-----------|-----|--------|---------|--------|------|
| off | 처리량(req/s) | 757.38 ± 3.20 | (기준) | | | | |
| Falco | 처리량(req/s) | 715.62 ± 2.81 | −5.51% | [−5.77, −5.26] | 0.0000 | 1.0000 | 차이 있음 |
| kShield-VirtualPatch | 처리량(req/s) | 755.08 ± 3.73 | −0.30% | [−0.60, −0.01] | 0.0428 | 0.0000 | 동등 |
| Tetragon | 처리량(req/s) | 699.56 ± 5.00 | −7.63% | [−7.99, −7.28] | 0.0000 | 1.0000 | 차이 있음 |
| off | 지연시간(ms) | 1.247 ± 0.005 | (기준) | | | | |
| Falco | 지연시간(ms) | 1.322 ± 0.005 | +6.04% | [+5.77, +6.32] | 0.0000 | 1.0000 | 차이 있음 |
| kShield-VirtualPatch | 지연시간(ms) | 1.250 ± 0.007 | +0.29% | [−0.03, +0.60] | 0.0719 | 0.0000 | 동등 |
| Tetragon | 지연시간(ms) | 1.351 ± 0.010 | +8.39% | [+7.96, +8.81] | 0.0000 | 1.0000 | 차이 있음 |

**[표13] p99 지연시간 및 도구 사용자 공간 자원(참고)**

| 그룹 | 워크로드 | p99 지연(ms) | CPU s/런 | RSS(MB) | 실패 요청 |
|------|---------|-------------|----------|---------|-----------|
| off | normal | 1.452 ± 0.047 | - | - | 0 |
| Falco | normal | 1.518 ± 0.038 | 0.148 | 327 | 0 |
| kShield-VirtualPatch | normal | 1.455 ± 0.046 | 0.000 | 3 | 0 |
| Tetragon | normal | 1.498 ± 0.039 | 0.483 | 112 | 0 |
| off | fork | 1.808 ± 0.067 | - | - | 0 |
| Falco | fork | 1.888 ± 0.084 | 0.522 | 327 | 0 |
| kShield-VirtualPatch | fork | 1.823 ± 0.088 | 0.000 | 3 | 0 |
| Tetragon | fork | 2.286 ± 0.173 | 2.715 | 300 | 0 |

**해석**: 동등성이 입증된 것은 kShield-VirtualPatch뿐이다 — normal 워크로드에서는 통계적으로 유의하지만(p<0.05) 사전에 고정한 ±2% 마진 안(TOST 유의), fork 워크로드에서는 유의성 자체가 사라지며 동등성이 입증되었다. Falco와 Tetragon은 두 워크로드 모두 마진을 벗어나는 유의한 오버헤드를 보였다. 애초 준비 단계에서는 "큰 차이가 없다"는 가설로 이 실험을 설계했으나(README 참고), 실측 결과는 그 가설을 지지하지 않는다 — 차이가 있고, 특히 Tetragon에서 fork 집약적 워크로드일 때 크게 벌어진다(처리량 −7.63%, `CPU s/런`이 normal 0.483s에서 fork 2.715s로 5.6배, RSS도 112MB에서 300MB로 증가). kShield-VirtualPatch의 `CPU s/런`이 두 워크로드 모두 0.000으로 나타나는 것은 측정 결함이 아니라 설계의 결과에 가깝다 — BPF 프로그램 자체의 실행 시간은 트리거한 프로세스(mock 서버)에 계상되고, 데몬은 perf buffer 이벤트만 읽는 최소 구현이기 때문이다.

이 차이를 "Tetragon이 느리다"로 단순화해서는 안 된다. Tetragon은 기본적으로 정책과 무관하게 프로세스 생명주기 전체를 관찰하는 범용 에이전트이며, Falco도 사용자 공간에서 이벤트를 필터링하는 범용 구조다. 두 도구 모두 이번 정책/룰 하나만이 아니라 훨씬 넓은 기능 범위를 상시 유지하는 비용을 함께 지고 있을 가능성이 높다(아래 대조군 실험으로 검증). kShield-VirtualPatch가 가벼운 것은 "더 빠른 구현"이어서가 아니라 AI 워커 계보라는 좁은 판정 하나만 커널 안에서 수행하도록 특화되어 있기 때문이며, 이는 범용성을 포기한 대가로 읽어야 한다 — Falco/Tetragon이 제공하는 룰 갱신·SIEM 연동·다중 판정 동시 운용 같은 기능은 kShield-VirtualPatch에 없다(2.3절).

**디스크 I/O로 원인 하나 더 확인**: 위 CPU·RSS만으로는 세 도구가 정확히 어떤 자원을 쓰는지 전부 드러나지 않는다. 같은 판정 로직을 별도 세션에서 재측정하면서 `/proc/{pid}/io`(런 전후 `read_bytes`/`write_bytes` 델타)를 추가로 잰 결과(표13-1), kShield-VirtualPatch와 Falco는 디스크 write가 사실상 0이었던 반면 **Tetragon만 normal 워크로드(요청 500회/런)에서 런당 약 1,491KB, fork 집약적 워크로드에서 약 17,134KB를 디스크에 썼다** — fork로 job당 프로세스 수가 늘어난 비율과 비슷하게 쓰기 양도 함께 늘었다.

**[표13-1] 도구 자신의 디스크 write(참고, 표13·14·15와 다른 별도 세션 — 절대 수치를 직접 비교하지 말 것)**

| 그룹 | 워크로드 | 디스크 write (KB/런) |
|------|---------|---------------------|
| Falco | normal | 0.0 |
| kShield-VirtualPatch | normal | 0.0 |
| Tetragon | normal | 1,491.0 |
| Falco | fork | 0.0 |
| kShield-VirtualPatch | fork | 0.0 |
| Tetragon | fork | 17,134.0 |

어디에 쓰는지 `/proc/<tetragon_pid>/fd`를 직접 열어 확인한 결과, `/var/log/tetragon/tetragon.log`(쓰기 전용 fd)로 밝혀졌다. 이 파일을 대상으로 (1) 아무 요청도 보내지 않은 대기 상태에서 10초 동안 6,735바이트(초당 약 673바이트)가 늘었고, (2) 목업 서버에 정상 job 50개를 보내자 183,116바이트(job당 평균 약 3,662바이트)가 추가로 늘었다. 즉 이 쓰기는 (a) 본 실험과 무관하게 VM 안의 다른 프로세스만으로도 상시 발생하고, (b) 우리 job이 만드는 fork/exec/exit 이벤트 수에 비례해서도 늘어난다 — `tetra getevents`로 누가 실제 구독하고 있는지와 무관하게(이 측정에서는 구독 클라이언트를 띄우지 않았다) 에이전트가 기본으로 켜 두는 로컬 export 로그로 보인다.

#### 4.5.1 오버헤드 원인 분리: 룰/정책의 비용인가, 에이전트 자체의 비용인가

위 해석은 "두 도구 모두 넓은 기능 범위를 상시 유지하는 비용을 지고 있을 가능성이 높다"는 추정에 기대고 있었다. 이 추정을 검증하지 않으면, 4.5절의 결과는 "AI 서버 전용으로 좁게 쓴 kShield-VirtualPatch의 룰이 애초에 더 가볍게 설계되었을 뿐"이라는 반박에 열려 있다 — 그렇다면 본 논문이 실제로 한 일이 "eBPF로 가상 패치를 구현"한 것이 아니라 "AI 서버 전용 탐지 룰을 하나 작성"한 것으로 축소된다. 이 가능성을 검증하기 위해 룰/정책을 제거한 대조군을 추가했다. `falco_norules`는 Falco를 결코 매칭되지 않는 더미 룰 하나(`proc.pid = -1`)로, `tetragon_norules`는 Tetragon을 TracingPolicy 없이(에이전트만) 기동한다 — 두 경우 모두 이벤트 스트림 자체(드라이버가 받는 이벤트 종류·양)는 `falco`/`tetragon` 그룹과 동일하게 켜져 있다. 나머지 측정 설계(off 기준, ABBA 교차, N=20, Bonferroni·TOST)는 4.5절 본 실험과 동일하다.

**[표14] 오버헤드 원인 분리 — normal 워크로드 (N=20, 기준 off, Bonferroni α=0.0031, 동등성 마진 ±2%)**

| 그룹 | 처리량(req/s) | Δ% | Welch p | TOST p | 판정 | CPU s/런 | RSS(MB) |
|------|--------------|-----|---------|--------|------|----------|---------|
| off | 745.96 ± 5.11 | (기준) | | | | - | - |
| Falco | 718.80 ± 3.69 | −3.64% | 0.0000 | 1.0000 | 차이 있음 | 0.149 | 327 |
| Falco(무룰) | 700.93 ± 5.81 | −6.04% | 0.0000 | 1.0000 | 차이 있음 | 0.636 | 328 |
| Tetragon | 726.70 ± 5.53 | −2.58% | 0.0000 | 0.9931 | 차이 있음 | 0.513 | 109 |
| Tetragon(무정책) | 729.28 ± 5.02 | −2.24% | 0.0000 | 0.8610 | 차이 있음 | 0.479 | 94 |

**[표15] 오버헤드 원인 분리 — fork 집약적 워크로드 (N=20, 기준 off)**

| 그룹 | 처리량(req/s) | Δ% | Welch p | TOST p | 판정 | CPU s/런 | RSS(MB) |
|------|--------------|-----|---------|--------|------|----------|---------|
| off | 751.40 ± 3.27 | (기준) | | | | - | - |
| Falco | 708.66 ± 3.14 | −5.69% | 0.0000 | 1.0000 | 차이 있음 | 0.533 | 327 |
| Falco(무룰) | 682.28 ± 6.29 | −9.20% | 0.0000 | 1.0000 | 차이 있음 | 0.890 | 331 |
| Tetragon | 692.97 ± 6.28 | −7.78% | 0.0000 | 1.0000 | 차이 있음 | 2.751 | 299 |
| Tetragon(무정책) | 693.35 ± 5.38 | −7.73% | 0.0000 | 1.0000 | 차이 있음 | 2.732 | 292 |

**해석**: 두 도구가 반대 양상을 보였다. **Tetragon**은 정책 유무와 무관하게 처리량·CPU·RSS가 거의 동일했다(normal 처리량 −2.58%→−2.24%, CPU 0.513s→0.479s, RSS 109MB→94MB; fork 처리량 −7.78%→−7.73%, CPU 2.751s→2.732s, RSS 299MB→292MB) — 오버헤드가 정책 내용이 아니라 에이전트가 정책과 무관하게 상시 수행하는 프로세스 생명주기 관측 자체에서 온다는 가설을 뒷받침한다. **Falco**는 반대였다 — 룰을 제거하자 오버헤드가 줄지 않고 오히려 커졌다(normal CPU 0.149s→0.636s, 4.3배; 처리량 −3.64%→−6.04%; fork CPU 0.533s→0.890s, 1.7배; 처리량 −5.69%→−9.20%). RSS는 룰 유무와 거의 무관했다(327MB↔328MB, 327MB↔331MB) — 에이전트가 유지하는 프로세스/컨테이너 상태 자체는 룰 내용과 무관하다는 뜻이다.

이 "정책과 무관한 상시 비용" 해석은 위 표13-1의 디스크 I/O 추가 측정으로도 뒷받침된다. 이번 perf 측정에는 TracingPolicy가 매치될 공격 시나리오 자체가 없었는데도 Tetragon은 계속 디스크에 썼고, 실제로 `/var/log/tetragon/tetragon.log`라는 구체적인 파일에 쓰고 있음을 직접 확인하였다 — 정책 내용과 무관한 비용이 CPU·RSS뿐 아니라 디스크 쓰기라는 물리적 형태로도 나타난다는 뜻이다. 다만 이 export 로그를 끄거나 로테이션을 설정했을 때 오버헤드가 줄어드는지는 검증하지 않았고(설정 가능 여부 포함 향후 과제), 측정 시점 기준 이 파일이 로테이션 없이 8.4MB 넘게 누적되어 있어 운영 시 디스크 사용량 관리도 함께 고려해야 함을 시사한다.

Falco 쪽 결과는 애초 세운 가설("룰을 걷어내면 가벼워질 것")과 반대였다. 정확한 원인은 이 실험 설계로는 확정할 수 없으나, 하나의 가능한 설명(추정, 미검증)은 이식한 실제 룰이 `evt.type = connect`/`evt.type in (execve, execveat)`로 이벤트 유형을 먼저 좁히는 반면, 이벤트 유형과 무관하게 항상 평가되는 더미 룰(`proc.pid = -1`)은 더 넓은 범위의 이벤트에 대해 룰 평가 자체가 수행되도록 만들었을 가능성이다 — 사실이라면 "룰이 좁을수록 가볍다"가 아니라 "룰이 이벤트 유형으로 얼마나 먼저 걸러지는가"가 관건이라는 뜻이 된다.

두 결과를 합치면 4.5절 서두의 추정을 반박하는 결론이 나온다: **kShield-VirtualPatch가 Falco/Tetragon보다 100배 가까이 가벼운 것은, AI 서버 전용으로 좁게 쓴 룰 덕분이 아니다.** Tetragon은 정책 내용과 무관하게 무겁고, Falco는 룰을 없애도 가벼워지지 않는다(오히려 무거워졌다) — 두 결과 모두 "룰을 좁게 썼기 때문에 가볍다"는 가설과 맞지 않는다. 이 격차는 세 도구의 **아키텍처 차이**에서 온다. Falco와 Tetragon도 kShield-VirtualPatch와 마찬가지로 eBPF 기반이므로(Falco는 modern eBPF 드라이버, Tetragon은 애초에 eBPF 전용으로 설계됨) 이 격차는 "eBPF 대 비eBPF"의 차이가 아니다. 차이는 두 도구가 *지금 로드된 룰/정책이 무엇이든 상관없이 앞으로 올 수 있는 임의의 룰을 지원*하기 위한 범용 관측 인프라(프로세스/컨테이너 상태 추적, 이벤트 스트림·gRPC 서버, 룰 엔진)를 상시 유지해야 하는 반면, kShield-VirtualPatch는 AI 워커 계보에 대한 고정된 판정 하나만 커널 안에서 내리고 매칭된 이벤트만 사용자 공간으로 올리는 최소 데몬이라는 데서 온다(3.2~3.3절). 즉 이 실험이 분리해 낸 것은 **범용 확장형 관측 플랫폼과 단일 고정 판정 간의 구조적 비용 차이**이며, 이는 2.3절·본 절 서두에서 제시한 "범용성의 비용 대 특화의 이점" 해석을 실측으로 뒷받침한다.

**범위와 한계**: (1) 감시 루트는 python3(본 논문 전체와 동일)이며 IPv4 한정이다. (2) Tetragon 정책에는 exec 계층이 없어 `nc`도 connect 단계에서 차단되므로, 표10의 "연결 수립" 차이는 도구의 우열이 아니라 계층 설계의 차이다. (3) kShield-VirtualPatch의 신뢰 IP·예외 UID·cgroup 예외·syslog 연동 같은 운영 기능(3.9~3.10절)은 이 실험이 비교하지 않는다 — 정성적으로만 2.3절에서 대비하였다. (4) 위 결과는 VM 1대·세션 1회의 측정이다. 4.4절의 절대 수치와 마찬가지로 세션 간 절대 비교는 하지 않았으며, 여러 세션에 걸친 반복 측정은 향후 연구로 남긴다(5장). (5) 4.5.1절의 Falco "이벤트 유형 필터링" 설명은 libsinsp 내부 계측으로 확인하지 않은 추정이다 — 코드 수준 검증은 향후 연구로 남긴다. (6) 표13-1의 디스크 I/O는 표13·14·15와 다른 별도 세션에서 추가로 측정한 것이라 CPU·RSS 절대 수치가 정확히 일치하지 않는다(이 세션의 Tetragon RSS는 normal 118MB·fork 470MB로, 표13의 112MB·300MB와 다르다) — 상대적 경향(정책 무관, Tetragon만 유의미한 디스크 비용)만 표13·14·15와 일관되게 재확인된 것으로 읽어야 한다.
...
이 재시작 동안 실제로 얼마나 무방비인지를 별도로 측정하였다(N=1, 단일 측정). Falco를 `kill -9`로 죽이고 곧바로 재기동시키는 순간부터 동일한 공격(`nc -w 2 203.0.113.1 80`)을 약 0.3초 간격으로 반복 투입하며 탐지 로그를 확인한 결과, 재시작 트리거로부터 3.009초 시점까지는 여전히 탐지되지 않았고 3.465초 시점에는 이미 탐지가 재개되어 있었다 — Falco 자체 로그에 찍힌 커널 이벤트 타임스탬프(`evt.time`)로 봐도 최초로 잡힌 이벤트는 재시작 트리거로부터 약 3.147초 뒤의 것이었다. 외부에서 폴링한 두 시점과 Falco 자체 타임스탬프가 모두 3.0~3.5초 사이로 일치하므로, 이 실험 설정 기준으로 Falco는 룰 하나를 반영하기 위한 재시작마다 **약 3초의 완전 무방비 구간**이 생기며, 이번 측정에서는 그 사이에 들어온 공격 시도 7회가 전부 놓쳤다. kShield-VirtualPatch·Tetragon은 룰/정책 반영에 재시작 자체가 없으므로 이 구간이 존재하지 않는다 — kShield-VirtualPatch는 3.9~3.10절의 런타임 제어 계층(`kshield_ctl`)으로, Tetragon은 `tracingpolicy add`로 각각 무재부팅 갱신을 지원한다. 다만 이는 VM 1대·세션 1회의 단일 측정이며, Falco 재시작 시간은 룰 파일 크기·시스템 부하에 따라 달라질 수 있어 반복 측정이 필요하다.
...
- **실제 Ray 클러스터 환경 검증의 확장**: 3.13절(v13)에서 실제 Ray 2.52.0 클러스터의 실제 Jobs API를 통해 정상 job 5회 성공·악성 job 1회 차단을 처음으로 확인하여, 이 항목은 기본적인 수준에서 해소되었다. 다만 (1) 검증한 entrypoint는 단순 서브프로세스 실행(`python3 -c ...`, `nc`)뿐이며, Ray actor·`@ray.remote` 클래스·멀티프로세싱 기반의 더 복잡한 job 실행 경로는 아직 검증하지 못했다 — 3.7절(v6)에서 다룬 것과 유사한 계보 추적 사각지대가 이런 경로에 다른 형태로 남아있을 가능성을 배제할 수 없다. (2) 단일 노드 클러스터·단일 세션 기준이며, 다중 노드 클러스터에서의 검증과 그때 필요한 신뢰 목록(각 노드 IP) 등록 부담의 정량화가 필요하다. (3) `kshield_vpatch_lsm`에 대해서는 아직 실제 Ray 클러스터 검증을 수행하지 않았다.
...
[2] Anyscale, "Update on Ray CVEs CVE-2023-6019, CVE-2023-6020, CVE-2023-6021, CVE-2023-48022, CVE-2023-48023," Anyscale Blog, Nov. 2023. [Online]. Available: https://www.anyscale.com/blog/update-on-ray-cves-cve-2023-6019-cve-2023-6020-cve-2023-6021-cve-2023-48022-cve-2023-48023
