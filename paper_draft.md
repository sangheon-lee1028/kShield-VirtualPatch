...
기획 관점에서 배포 시나리오를 재검토하는 과정에서 마지막으로 발견한 공백은(3.8절, v7), 계보 추적 맵이 오직 fork 훅 발동 시점에만 채워진다는 설계가 "데몬이 클러스터보다 나중에 뜨거나 재시작된다"는 상황을 고려하지 않았다는 점이었다. 이 경우 데몬 기동 이전에 이미 fork가 끝난 자손 프로세스는 스스로 다시 fork하기 전까지 계보에서 누락된다. 커널 BPF 프로그램은 그대로 두고 유저스페이스 로더가 기동 시 `/proc`을 스캔해 이미 실행 중인 프로세스들의 계보를 동일한 맵에 직접 채워 넣도록 수정하였으며, 직속 부모 검사만으로는 절대 잡을 수 없는 손자뻘(2단계) 프로세스 체인 — 데몬 기동 전에 미리 띄워 둔 뒤 뒤늦게 의심 행위를 하는 시나리오 — 으로 VM에서 재현·검증하였다.
