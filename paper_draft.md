...
**남는 한계**: (1) 호스트 자체가 재부팅되는 경우는 다루지 않는다 — `bpffs`는 보통 메모리 기반이라 재부팅되면 핀도 함께 사라지므로, 이 수정이 보장하는 것은 "데몬만 크래시하는" 상황에 한정된다(두 컴포넌트 공통). (2) 재시작 시 "새 attach 생성 → 옛 핀 제거"라는 교체 순서 자체가 진짜로 무공백인지는 코드 로직으로 추론한 것이며, 그 교체 찰나의 마이크로초 단위 공백 유무를 별도로 계측하지는 않았다 — 이번에 직접 측정한 것은 "데몬이 재시작 없이 계속 죽어 있는 동안"의 창(kill -9 후 2초+)이다. (3) 두 컴포넌트 모두 Tetragon의 `Restart=on-failure` 같은 자동 재시작 기능이 없다 — 이번 수정은 "재시작 전까지 계속 막아준다"는 것이지 "알아서 재시작된다"는 것은 아니며, 자동 재시작은 systemd 유닛 등 별도 운영 설정의 몫으로 남겨 둔다.
...
- **재시작 핸드오프 순간의 무공백 여부 정밀 계측**: v11(3.11절)은 "데몬이 재시작 없이 계속 죽어 있는 동안"은 직접 측정했지만(kill -9 후 2초+), 재시작 그 자체의 "새 attach 생성 → 옛 핀 제거" 교체 찰나에 마이크로초 단위 공백이 있는지는 코드 로직으로만 추론하였다 — `bpf_ktime_get_ns()` 기반의 별도 계측이 필요하다.
