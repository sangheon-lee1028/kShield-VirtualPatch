...
본 절 전체(3.3, 3.5~3.12절)에 걸쳐 설계가 v1부터 v12까지 총 열한 차례 반복되었다. 각 반복은 예외 없이 VM 실측이나 제출 전 자체 검토 과정에서 발견된 구체적 문제에 대한 대응이었으며, [표 1]은 이를 한눈에 볼 수 있도록 요약한 것이다 — 각 행의 상세 서술은 마지막 열의 절을 참고한다.
...
- **실제 Ray 클러스터 환경 검증**: 공격 탐지·성능 실험은 여전히 mock 서버 기반 재현이며, 실제 Ray 설치 환경에서의 검증이 필요하다. mock 서버는 entrypoint를 항상 서브프로세스로 실행하므로, 실제 Ray의 job 실행 경로(멀티프로세싱, actor 기반 실행 등)가 이와 다르다면 3.7절(v6)에서 다룬 것과 유사한 계보 추적 사각지대가 다른 형태로 남아있을 가능성을 배제할 수 없다. 다만 3.1절의 토큰 인증 검증은 실제 Ray 2.52.0 설치본을 대상으로 수행하였다 — 인증 계층 자체의 동작은 실측했지만, kShield-VirtualPatch의 탐지 로직(SHADOW_EXEC/CONNECT)을 실제 Ray의 job 실행 경로에 대해 검증한 것은 아니므로 이 항목은 부분적으로만 해소되었다.
