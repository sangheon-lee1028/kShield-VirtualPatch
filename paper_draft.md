...
- **`watched_self[]`의 kprobe/LSM 훅 타이밍 비대칭 해소**: 3.11절 재측정 중, fork 없이 자기 자신을 `execve()`로 치환하는 프로세스(예: `bash -c '<단일 명령>'`)에 대해 `watched_self[]`(3.7절, v6) 검사가 LSM 컴포넌트(`execve()` 이전 발동)에서는 성공하지만 kprobe 컴포넌트(`tp/sched/sched_process_exec`, `execve()` 이후 발동이라 `comm`이 이미 바뀐 상태)에서는 실패한다는 것이 새로 확인되었다. 같은 판정 로직이 훅의 실행 시점 차이만으로 한쪽 컴포넌트에서 무력화되는 사례이며, kprobe 컴포넌트에 self-exec 치환을 별도로 포착하는 보완 로직(예: `execve` 진입 시점의 이전 `comm`을 별도로 추적)이 필요하다.
