...
**남는 한계**: (1) 이번 수정과 검증은 `kshield_vpatch`(SHADOW_EXEC/CONNECT, 3.3절)에만 적용하였다 — `kshield_vpatch_lsm`(3.5절)에 같은 fail-open 문제가 있는지는 이번 라운드에서 확인하지 않았다. (2) 호스트 자체가 재부팅되는 경우는 다루지 않는다 — `bpffs`는 보통 메모리 기반이라 재부팅되면 핀도 함께 사라지므로, 이 수정이 보장하는 것은 "데몬만 크래시하는" 상황에 한정된다. (3) 재시작 시 "새 attach 생성 → 옛 핀 제거"라는 교체 순서 자체가 진짜로 무공백인지는 코드 로직으로 추론한 것이며, 그 교체 찰나의 마이크로초 단위 공백 유무를 별도로 계측하지는 않았다 — 이번에 직접 측정한 것은 "데몬이 재시작 없이 계속 죽어 있는 동안"의 창(kill -9 후 2초+)이다. (4) kShield-VirtualPatch 자체에는 Tetragon의 `Restart=on-failure` 같은 자동 재시작 기능이 없다 — 이번 수정은 "재시작 전까지 계속 막아준다"는 것이지 "알아서 재시작된다"는 것은 아니며, 자동 재시작은 systemd 유닛 등 별도 운영 설정의 몫으로 남겨 둔다.
...
- **`kshield_vpatch_lsm`의 fail-open 여부 검증**: v11(3.11절)은 `kshield_vpatch`(SHADOW_EXEC/CONNECT)의 BPF link만 bpffs에 핀하도록 고쳤다. `kshield_vpatch_lsm`(3.5절)의 동기적 사전 차단 훅도 같은 방식(libbpf skeleton attach)으로 부착되므로 동일한 fail-open 위험이 있을 수 있으나, 이번 라운드에서는 확인·수정하지 않았다.
