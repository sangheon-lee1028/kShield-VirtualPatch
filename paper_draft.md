...
mock Ray Jobs API 환경에서 python3→sh→curl로 이어지는 2단계 공격 체인은 curl 실행 시점에(SHADOW_EXEC), bash `/dev/tcp/` 기반 우회 시도는 연결 시도 시점에(SHADOW_CONNECT) 각각 즉시 SIGKILL로 차단되었으며, 두 경우 모두 정상 job(echo, python3 -c 등)은 오탐 없이 통과하였다. 비활성/SHADOW_EXEC+CONNECT/LSM 사전 차단까지 포함한 3개 그룹을 10회씩 반복 측정한 결과, fork 단계가 얕은 워크로드에서는 세 그룹 간 통계적으로 유의미한 처리량·지연시간 차이가 관측되지 않았다(비활성 대비 SHADOW_EXEC+CONNECT: 처리량 p=0.0864, 지연시간 p=0.1400; SHADOW_EXEC+CONNECT 대비 LSM 추가: 처리량 p=0.6228, 지연시간 p=0.9348; Welch's t-test). 다만 job당 fork를 50배가량 늘린 워크로드로 재측정하자 처음으로 통계적으로 유의미한(그러나 작은, 처리량 −0.67%·지연시간 +0.7%) 차이가 나타나, 오버헤드가 워크로드의 fork 빈도에 비례함을 확인하였다. 다만 SHADOW_EXEC/SHADOW_CONNECT는 모두 행위 발생 "이후" `bpf_send_signal`로 비동기 종료시키는 방식이라는 잔여 한계가 있어, 이를 해소하기 위해 `security_bprm_check_security`/`security_socket_connect` LSM 훅으로 시스템 콜 자체를 동기적으로 실패시키는 사전 차단 컴포넌트를 추가로 구현하고 VM에서 기능·성능 검증하였다(3.5절). 이 과정에서 실행 파일 블록리스트가 `/usr/bin/curl`은 막으면서 동일 파일을 가리키는 `/bin/curl`은 놓치는 버그를 실측으로 발견하였으며 — 셸의 `$PATH` 탐색이 거부된 경로 대신 다른 경로 문자열로 재시도한 것이 원인이었다 — 이는 이미 검증되었던 v3(SHADOW_EXEC)에도 동일하게 존재하던 누락이라 함께 수정하였다.

이후 SHADOW_EXEC의 오탐 전제 자체를 재검토하였다(3.6절). curl/wget은 실제 AI 서빙 워크로드에서 모델·데이터셋 다운로드에도 쓰이는 이중 용도(dual-use) 도구여서, 실행 파일 이름만으로 목적지와 무관하게 즉시 차단하면 정당한 다운로드 job까지 오탐으로 죽인다는 점을 확인하였다. curl/wget을 실행 즉시 차단 목록에서 제외하고 판단을 목적지 인지 계층(SHADOW_CONNECT/LSM socket_connect)에 전적으로 넘기도록 재설계하였으며, VM 실측으로 신뢰 목적지로의 정상 다운로드는 통과하고 신뢰되지 않은 목적지로의 시도는 여전히(exec이 아닌 connect 시점에) 차단됨을 확인하였다. 아울러 신규 룰을 먼저 "탐지만 하고 차단은 안 함"으로 배포한 뒤 오탐을 관찰하고 나서 실제 차단으로 전환할 수 있도록, 재컴파일 없이 실행 시점에 전환 가능한 audit-only 모드를 추가하고 v3·LSM 컴포넌트 양쪽에서 VM 검증하였다.

제출 전 자체 검토 과정에서 계보 판정 로직 자체의 구조적 공백도 발견하였다(3.7절, v6). 계보 추적은 "감시 대상 프로세스의 자손"만 등록하도록 되어 있어, 감시 대상 프로세스 자신(예: `raylet`)이 fork 없이 직접 `execve()`/`connect()`를 호출하면 어느 훅도 이를 감지하지 못하는 맹점이 있었다 — 지금까지의 실험은 mock 서버가 entrypoint를 항상 서브프로세스로 실행했기 때문에 이 경로가 한 번도 노출되지 않았었다. 감시 대상 프로세스명만 담은 별도 목록(`watched_self[]`)으로 자기 자신의 comm도 확인하도록 수정하였으며, 실제 바이너리의 파일명만 감시 대상 이름으로 바꿔 fork 없이 직접 실행하는 시나리오로 VM에서 재현·검증하였다 — 이름을 바꾼 프로세스는 정확히 잡히고, 이름을 바꾸지 않은 무관한 프로세스는 영향받지 않음을 확인하였다.
...
- **LSM 기반 사전 차단의 반복 검증**: `kshield_vpatch_lsm`(3.5절)은 빌드·attach·기능 검증과 SIGKILL 방식(3.3절) 대비 성능 오버헤드 측정(4.4절)까지 VM에서 완료하였으나, N회 반복 탐지율 측정은 아직 수행하지 않았다.
...
- **실제 Ray의 fork 패턴 재현**: 본 실험의 fork 집약적 워크로드(`/bin/true` 반복 실행)는 실제 Ray 태스크의 워커 프로세스 생성 패턴을 정밀하게 재현한 것은 아니다. 실제 Ray 클러스터(향후 연구 항목 참고)의 태스크 처리량·fork 빈도를 프로파일링하여 더 현실적인 워크로드로 재측정할 필요가 있다.
...
- **오탐 시나리오 확장 검증**: `echo`, `python3 -c` 같은 단순 job 외에, curl/wget을 이용한 정상 다운로드 시나리오는 3.6절에서 실측으로 검증하였다. 다만 AI 워커가 그 밖의 서브프로세스를 실행하거나 외부와 통신하는 더 복잡한 합법적 워크로드(예: 데이터 전처리 파이프라인, pip/conda 패키지 설치, 정상적인 API 호출)와의 충돌 가능성은 아직 다양한 시나리오로 검증하지 못했다.
